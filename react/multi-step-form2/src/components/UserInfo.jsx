...
const UserInfo = ({ setCurrentStep }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const  nameInput = validateName(data.get("name"));
    const  emailInput = validateEmail(data.get("email"));
    const  phoneInput = validatePhone(data.get("phone"));
  ...
  return (
    <div className="bg-white mr-4 ml-4 px-6 py-8 mb-6 absolute rounded-lg shadow-md">
      ...
      <p className=" w-full mb-6 text-gray-500 mb-6">Please provide your name, email address, and phone number.</p>

      <form id="personal-info-form" onSubmit={() => handleSubmit}>

        <div>
          <label htmlFor="name" className="label body-sm">Name</label>
          <input type="text" name="name" id="name" className="input" placeholder="e.g. Stephen King"/>
          {errors.name && (
          <p className="text-red-500 bold">{errors.name}</p>
          )}  
        </div>

        <div>
          <label htmlFor="email" className="label body-sm">Email Address</label>
          <input type="email" name="email" id="email" className="input" placeholder="e.g. stephenking@lorem.com" />
          {errors.mail && (
          <p className="text-red-500 bold">{errors.mail}</p>
          )}
        </div>

        <div>
          <label htmlFor="phone" className="label body-sm">Phone Number</label> 
          <input type="tel" name="phone" id="phone" className="input text-center" placeholder="e.g. +1 234 567 890"/>
          {errors.phone && (
            <p className="text-red-500 bold">{errors.phone}</p>
          )}
        </div>
      </form>
